...
namespace AdventOfCode.Day07
{
    public class Operations
    {
        ...
        public Operations(ITestOutputHelper console)
        {
            _console = console;
        ...
        private int Add(List<int> memory, List<int> parameters, int currentAddress)
        {
            ...
            _console.WriteLine($"[{currentAddress}] Set {parameters[2]} to {parameters[0] + parameters[1]} ( {parameters[0]} + {parameters[1]} )");
        ...
        private int JumpIfNotZero(List<int> memory, List<int> parameters, int currentAddress)
        {
            if (parameters[0] != 0)
            {
                _console.WriteLine($"[{currentAddress}] Jump to {parameters[1]} ( {parameters[0]} != 0 ) if (non-zero)");
                return parameters[1];
            }

            _console.WriteLine($"[{currentAddress}] Ignore Jump ( {parameters[0]} == 0 ) if (non-zero)");
        ...
        private int JumpIfEqualZero(List<int> memory, List<int> parameters, int currentAddress)
        {
            if (parameters[0] == 0)
            {
                _console.WriteLine($"[{currentAddress}] Jump to {parameters[1]} ( {parameters[0]} == 0 ) if (equal zero)");
                return parameters[1];
            }

            _console.WriteLine($"[{currentAddress}] Ignore Jump ( {parameters[0]} != 0 ) if (equal zero)");
        ...
        private int LessThan(List<int> memory, List<int> parameters, int currentAddress)
        {
            if (parameters[0] < parameters[1])
            {
                ...
                _console.WriteLine($"[{currentAddress}] Set {parameters[2]} to 1 ( {parameters[0]} < {parameters[1]} )");
            }
            else
            {
                memory[parameters[2]] = 0;

                _console.WriteLine($"[{currentAddress}] Set {parameters[2]} to 0 ( {parameters[0]} NOT < {parameters[1]} )");
            }
        ...
        private int EqualTo(List<int> memory, List<int> parameters, int currentAddress)
        {
            ...
            else
            {
                ...
                _console.WriteLine($"[{currentAddress}] Set {parameters[2]} to ) ( {parameters[0]} != {parameters[1]} )");
            }
